...
`LifeCycleSetup` <- function( dataset=c( "RNAseq", "YoungWinzeler", "DeRisi", "Custom"), 
	...
	if ( dataset == "RNAseq") {
		...
		ans <- buildLifeCycleVectorSpace( file=NULL, tbl=lifeCycleData, geneColumn="Gene",
			...
			#"Schizont"=schizPtrs, "EarlyGameto"=gcyte2Ptrs, "LateGameto"=gcyte5Ptrs, 
			#"Ookinete"=ookinePtrs, "OocystSPZ"=oospzPtrs), 
			dimensions=list( "SalivarySPZ"=sgspzPtrs, "LiverDay3"=lsd3Ptrs, "Merozoite"=merozPtrs,
			"EarlyRing"=eringPtrs, "LateRing"=lringPtrs, "EarlyTroph"=etrophPtrs, "LateTroph"=ltrophPtrs, 
...
# we are making a set of routines "just like" the cell type tools, but for life cycle


`gene2LifeCycle` <- function( genes, speciesID=getCurrentSpecies()) {
	...
	geneLifeCycle <- NULL
	data( list=paste( prefix,"GeneLifeCycle",sep="."), package="DuffyTools", envir=environment())
	if ( is.null(geneLifeCycle)) {
		cat( "\nWarning:  No 'GeneLifeCycle' object loaded for species: ", speciesID)
		return( out)
	}

	where <- match( genesIn, geneLifeCycle$GENE_ID, nomatch=0)
	out[ where > 0] <- geneLifeCycle$LifeCycle[ where]
...
`lifeCycleEnrichment` <- function( lifeCycle, mode=c("geneSets", "genes"), 
	...
	if ( mode == "geneSets") {
		...
		data( list="GeneSetLifeCycle", package="DuffyTools", envir=environment())
		if ( is.null(geneSetLifeCycle)) stop( "'GeneSetLifeCycle' object not loaded.")
		lifeCycleUniverse <- geneSetLifeCycle$LifeCycle
	}
	if ( mode == "genes") {
		...
		geneLifeCycle <- NULL
		data( list=paste( prefix,"GeneLifeCycle",sep="."), package="DuffyTools", envir=environment())
		if ( is.null(geneLifeCycle)) stop( "'GeneLifeCycle' object not loaded.")
		...
		dups <- which( duplicated( geneLifeCycle$GENE_ID))
		if ( length(dups)) geneLifeCycle <- geneLifeCycle[ -dups, ]
		...
		if ( ! is.null( geneUniverse)) {
			...
			keep <- which( geneLifeCycle$GENE_ID %in% geneUniverse)
			if (verbose) cat( "\nReducing Gene Universe from ", nrow(geneLifeCycle), " to ", length(keep))
			geneLifeCycle <- geneLifeCycle[ keep, ]
		}

		# now we know how big the universe of genes is
		lifeCycleUniverse <- geneLifeCycle$LifeCycle
	}

	# any missing cell types in the input should not influence our counts or our enrichment calls
	# remove them
	drops <- c( which( lifeCycle == ""), which( is.na( lifeCycle)))
	if ( length( drops)) lifeCycle <- lifeCycle[ -drops]

	# ready to calculate enrichment
	givenTbl <- table( lifeCycle)
	givenPcts <- givenTbl * 100 / sum(givenTbl)
	givenNames <- names( givenTbl)
	allTbl <- table( lifeCycleUniverse)
	...
	N_All <- length( lifeCycleUniverse)
	N_Given <- length( lifeCycle)
	...
	colnames( out) <- c( "LifeCycle", "Enrichment", "P_Value", "N_Total", "Pct_Total", "N_Given", "Pct_Given")
...
lifeCycleExpression <- function( m, mode=c("sqrtmean","log2")) {

	# given a matrix of gene expression, reduce it to a matrix of cell type expression
	gids <- shortGeneName( rownames(m), keep=1)

	cids <- gene2LifeCycle( gids)
	lifeCycle <- sort( unique( cids))
	NCT <- length( lifeCycle)
	if ( NCT < 2) {
		cat( "\nUnexpected low number of life cycles: ", NCT)
		cat( "\nDebug:  Gene IDs rownames(m): ", head( gids))
		cat( "\nDebug:  Life Cycle calls:      ", head( cids))
		return( NULL)
	}

	out <- matrix( NA, nrow=NCT, ncol=ncol(m))
	colnames(out) <- colnames(m)
	rownames(out) <- gsub( "\\.\\.+", ".", make.names( lifeCycle))
